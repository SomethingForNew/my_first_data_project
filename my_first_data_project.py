...
df = pd.read_csv('/kaggle/input/my-data/sales_data.csv')
...
# 위 코드를 sql으로 이해해보면 select sum(quantity) from sales_data group by menu_item 이다.
...
# 위에서 groupby를 사용하고 그래프 그리기 방식1
# 데이터 처리와 시각화가 분리되어 매우 명확하다.
# Matplotlib은 세부적인 커스터마이징이 가능하지만, Seaborn에 비해 디자인이 투박하고, 코드 작성이 더 복잡해질 수 있다.
# plt.bar(x = menu_sales.index, height = menu_sales.values)

# 위에서 groupby를 사용하고 그래프 그리기 방식2
...
# plt.show() # 로컬에서 .py 파일 실행시 그래프를 보기위한 창을 띄워준다.
